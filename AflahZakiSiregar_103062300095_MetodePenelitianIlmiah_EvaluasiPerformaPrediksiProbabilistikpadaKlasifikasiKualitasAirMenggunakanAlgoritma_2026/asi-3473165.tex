...
% Penanda perubahan: teks/nilai yang diubah ditandai dengan \changed{...}
% Hapus definisi ini untuk menghilangkan highlight pada versi final
...
Li et al. \cite{li2024battery} mengimplementasikan NGBoost untuk estimasi \textit{State of Charge} (SoC) baterai secara probabilistik, membuktikan kemampuan model dalam menghasilkan prediksi terkalibrasi dengan interval kepercayaan. Pada domain tersebut, ketidakpastian pengukuran sensor yang tinggi menuntut pendekatan yang mampu mengkuantifikasi ketidakpastian prediksi, di mana NGBoost terbukti lebih andal dibandingkan estimasi deterministik. Sementara itu, Zhu et al. \cite{zhu2023hybrid} mengintegrasikan SMOTE-ENN dengan NGBoost untuk prediksi risiko finansial, di mana pendekatan probabilistik tersebut mampu mengkuantifikasi ketidakpastian pada data dengan kelas tidak seimbang. Marzo et al. \cite{marzo2021ngboost} juga mengadaptasi NGBoost untuk prediksi mutasi somatik pada domain biomedis, mengonfirmasi fleksibilitas algoritma ini dalam menghasilkan estimasi probabilistik pada berbagai konteks aplikasi. Studi-studi tersebut memberikan indikasi kuat bahwa pendekatan probabilistik, sebagaimana diilustrasikan oleh NGBoost, dapat diadaptasi untuk domain kualitas air di mana ketidakpastian parameter fisikokimia menjadi karakteristik inheren.

Berdasarkan kajian terhadap penelitian-penelitian terdahulu, diketahui bahwa domain kualitas air masih didominasi oleh pendekatan deterministik yang tidak mengakomodasi ketidakpastian prediksi. Selain studi-studi di atas, penelitian oleh Ahmed et al. \cite{ahmed2022water} dan Uddin et al. \cite{uddin2022review} secara komprehensif meninjau penerapan \textit{machine learning} pada pemantauan kualitas air dan mengonfirmasi dominasi pendekatan deterministik. Khoi et al. \cite{khoi2022water} dan Sakaa et al. \cite{sakaa2022water} juga menerapkan model \textit{ensemble} untuk prediksi indeks kualitas air tanpa menyertakan kuantifikasi ketidakpastian probabilistik. Tabel~\ref{tab:perbandingan_penelitian} menyajikan perbandingan komprehensif terhadap penelitian terdahulu berdasarkan dimensi metodologi, tipe prediksi, dan domain aplikasi.
...
Kalibrasi probabilitas merupakan aspek kritis dalam evaluasi model prediktif probabilistik. Sebuah model dikatakan terkalibrasi apabila probabilitas prediksi yang dihasilkan mencerminkan frekuensi kejadian aktual dengan akurat \cite{degroot1983forecasters, platt1999probabilistic, zadrozny2002transforming, kuleshov2018calibration}. Misalnya, jika model memprediksi probabilitas $0.8$ untuk sekelompok instance, maka sekitar $80\%$ dari instance tersebut seharusnya benar-benar termasuk kelas positif.
...
Implementasi direncanakan dalam lingkungan Python 3.9+ dengan pustaka utama meliputi \texttt{ngboost} untuk pemodelan probabilistik, serta \texttt{scikit-learn} \cite{pedregosa2011scikit}, \texttt{pandas}, \texttt{numpy}, \texttt{matplotlib}, dan \texttt{seaborn} untuk preprocessing, evaluasi, dan visualisasi. Lingkungan pengembangan menggunakan Jupyter Notebook atau Google Colab dengan akses GPU/TPU opsional untuk mempercepat hyperparameter tuning.
...
Nilai hilang terutama ditemukan pada fitur pH, \textit{Sulfate}, dan \textit{Trihalomethanes}. Untuk mengatasinya, diimplementasikan metode \textit{Multivariate Imputation by Chained Equations} (MICE), yaitu pendekatan imputasi berbasis regresi iteratif yang memanfaatkan hubungan antar fitur dalam memperkirakan nilai yang hilang \cite{buuren2011mice, rubin1987multiple, white2011multiple}. Metode ini dipilih karena karakteristik fitur pada dataset kualitas air tidak sepenuhnya independen secara kimiawi, sehingga imputasi berbasis statistik sederhana berpotensi mengabaikan relasi antar variabel. Imputasi dilakukan pada seluruh dataset sebelum pembagian data untuk memastikan konsistensi struktur data.
...
Teknik SMOTE-ENN dipertimbangkan secara kondisional berdasarkan hasil EDA yang menunjukkan ketidakseimbangan proporsi antar kelas. SMOTE-ENN merupakan pendekatan hibrida yang menggabungkan \textit{oversampling} kelas minoritas dan \textit{undersampling} sampel \textit{noise} di area batas keputusan \cite{batista2004study, chawla2002smote}. Penerapan dibatasi hanya pada data latih yang telah diskalakan untuk mencegah \textit{data leakage} dan memastikan stabilitas pembentukan sampel sintetik. Apabila \textit{baseline} menunjukkan performa memadai tanpa resampling, dataset asli tetap digunakan.
...
Kriteria pemilihan konfigurasi final didasarkan pada \textit{trade-off} antara penurunan NLL dan stabilisasi ECE pada data validasi. Konfigurasi optimal dipilih sebagai kombinasi yang menghasilkan keduanya dalam kondisi seimbang. \changed{Mekanisme \textit{early stopping} diterapkan pada proses \textit{training} NGBoost untuk mencegah \textit{overfitting}, di mana iterasi boosting dihentikan ketika stabilisasi NLL pada data validasi tidak menunjukkan penurunan signifikan pada iterasi-iterasi berikutnya. Untuk XGBoost, seluruh iterasi dijalankan dengan monitoring validasi loss untuk mengamati dinamika konvergensi}.
...
Untuk menilai signifikansi statistik perbedaan akurasi antar pasangan model, digunakan uji McNemar \cite{dietterich1998approximate}. Uji ini beroperasi pada hasil prediksi berpasangan (\textit{paired binary outcomes}) berupa benar/salah dari dua model pada instance data uji yang sama, sehingga sesuai untuk membandingkan klasifier yang dievaluasi pada satu set pengujian identik. Uji McNemar dipilih karena tidak mengasumsikan independensi antar sampel dan mampu mendeteksi perbedaan performa yang konsisten meskipun marginal secara agregat.
...
Berdasarkan eksplorasi data awal (EDA), dataset \textit{Water Potability} terdiri atas 3.276 sampel dengan 9 fitur fisikokimia dan 1 variabel target biner. Distribusi kelas menunjukkan ketidakseimbangan yang signifikan: 1.998 sampel (60,99\%) merupakan kelas tidak layak konsumsi (\textit{not potable}) dan 1.278 sampel (39,01\%) termasuk kelas layak konsumsi (\textit{potable}). Ketidakseimbangan ini mengonfirmasi perlunya penanganan \textit{class imbalance} pada tahap \textit{preprocessing} sebagaimana direncanakan dalam metodologi. Visualisasi distribusi kelas disajikan pada Gambar~\ref{fig:class_distribution}.
...
\begin{tikzpicture}
    ...
    \draw[->, thick] (0,0) -- (8.5,0) node[right, font=\small] {Jumlah Sampel};
    \draw[->, thick] (0,0) -- (0,3.2);
    % Not Potable bar (1998 samples, ~61%)
    \fill[red!40] (0,1.8) rectangle (7.0,2.6);
    \draw[black] (0,1.8) rectangle (7.0,2.6);
    \node[left, font=\small] at (0,2.2) {\textit{Not Potable}};
    \node[right, font=\small] at (7.1,2.2) {1.998 (60,99\%)};
    % Potable bar (1278 samples, ~39%)
    \fill[blue!40] (0,0.5) rectangle (4.47,1.3);
    \draw[black] (0,0.5) rectangle (4.47,1.3);
    \node[left, font=\small] at (0,0.9) {\textit{Potable}};
    \node[right, font=\small] at (4.57,0.9) {1.278 (39,01\%)};
    % X-axis ticks
    \draw (0,-0.1) -- (0,0.1); \node[below, font=\scriptsize] at (0,-0.15) {0};
    \draw (1.75,-0.1) -- (1.75,0.1); \node[below, font=\scriptsize] at (1.75,-0.15) {500};
    \draw (3.5,-0.1) -- (3.5,0.1); \node[below, font=\scriptsize] at (3.5,-0.15) {1000};
    \draw (5.25,-0.1) -- (5.25,0.1); \node[below, font=\scriptsize] at (5.25,-0.15) {1500};
    \draw (7.0,-0.1) -- (7.0,0.1); \node[below, font=\scriptsize] at (7.0,-0.15) {2000};
\end{tikzpicture}%
}
\caption{Distribusi Kelas pada Dataset \textit{Water Potability}. Ketidakseimbangan antara kelas \textit{Not Potable} (60,99\%) dan \textit{Potable} (39,01\%) \changed{memotivasi evaluasi penanganan \textit{class imbalance} menggunakan SMOTE-ENN pada data latih}.}
\label{fig:class_distribution}
\end{figure}

Pemeriksaan nilai hilang mengidentifikasi tiga fitur dengan proporsi \textit{missing values} yang signifikan, sebagaimana disajikan pada Tabel~\ref{tab:missing_values}. Fitur \textit{Sulfate} memiliki persentase nilai hilang tertinggi (23,84\%), diikuti oleh pH ($14{,}99$\%) dan \textit{Trihalomethanes} ($4{,}95$\%). Keenam fitur lainnya tidak memiliki nilai hilang. Imputasi menggunakan MICE berhasil mengisi seluruh nilai hilang dengan mempertahankan struktur korelasi multivariat antar parameter fisikokimia, sebagaimana direkomendasikan oleh van Buuren dan Groothuis-Oudshoorn \cite{buuren2011mice}.
...
\textit{Trihalomethanes}    & 162 & $4{,}95$  & 0 \\
...
Setelah imputasi, dataset dibagi menggunakan \textit{stratified split} dengan proporsi 70:15:15, menghasilkan $2.292$ sampel data latih, $492$ sampel data validasi, dan 492 sampel data uji. Stratifikasi berhasil mempertahankan distribusi kelas pada masing-masing subset, dengan rasio tidak layak/layak masing-masing sebesar $\approx$61\%/39\%.

\changed{Sesuai dengan pendekatan kondisional yang diuraikan dalam metodologi, teknik SMOTE-ENN dievaluasi pada data latih. Hasil evaluasi menunjukkan bahwa SMOTE-ENN justru menurunkan jumlah sampel latih dari 2.292 menjadi 1.096 (pengurangan 1.196 sampel akibat dominasi fase ENN) dan menurunkan akurasi seluruh model secara signifikan: NGBoost turun dari 0,665 menjadi 0,549; XGBoost dari 0,665 menjadi 0,577; dan Random Forest dari 0,659 menjadi 0,585. Berdasarkan degradasi performa ini, SMOTE-ENN tidak diterapkan pada pelatihan model final, dan model-model dilatih menggunakan data latih asli tanpa resampling}.
...
\midrule
NGBoost                & $0{,}665$ & $\textbf{0{,}667}$ & $0{,}281$ & $0{,}396$ & $0{,}673$ & $0{,}069$ \\
XGBoost                & $\textbf{0{,}665}$ & $0{,}634$ & $\textbf{0{,}333}$ & $\textbf{0{,}437}$ & $0{,}624$ & $0{,}058$ \\
\textit{Random Forest} & $0{,}659$ & $0{,}630$ & $0{,}302$ & $0{,}409$ & $\textbf{0{,}618}$ & $\textbf{0{,}042}$ \\
\bottomrule
\end{tabular}%
}
\end{table}

Ditinjau dari metrik klasifikasi diskriminatif, \changed{NGBoost dan XGBoost mencatat akurasi yang identik sebesar 66,5\%, diikuti \textit{Random Forest} (65,9\%). Ketiga model menunjukkan performa diskriminatif yang sangat serupa, mengindikasikan bahwa perbedaan algoritmik tidak menghasilkan keunggulan akurasi yang berarti pada dataset ini}. Pada dataset dengan ketidakseimbangan kelas, \textit{F1-Score} merupakan metrik yang lebih representatif dari akurasi. \changed{\textit{F1-Score} NGBoost sebesar 0,396 lebih rendah dibandingkan XGBoost (0,437) dan \textit{Random Forest} (0,409), yang disebabkan oleh rendahnya recall NGBoost (0,281). NGBoost cenderung menghasilkan prediksi positif yang lebih konservatif dengan presisi tertinggi (0,667) namun mengorbankan sensitivitas terhadap kelas positif}.

Untuk memverifikasi signifikansi statistik perbedaan akurasi antar model, dilakukan uji McNemar pada pasangan prediksi model \cite{dietterich1998approximate, demsar2006statistical}. \changed{Hasil pengujian menunjukkan: NGBoost vs XGBoost menghasilkan $p = 0{,}905$; NGBoost vs \textit{Random Forest} menghasilkan $p = 0{,}791$; dan XGBoost vs \textit{Random Forest} menghasilkan $p = 0{,}760$}. \changed{Seluruh nilai $p$ berada jauh di atas ambang signifikansi $\alpha = 0{,}05$}, sehingga perbedaan akurasi antar ketiga model tidak signifikan secara statistik. \changed{Temuan ini mengonfirmasi bahwa ketiga model memiliki kemampuan diskriminatif yang setara pada dataset ini}.
...
Analisis pada tingkat per-kelas ditunjukkan melalui matriks konfusi komparatif (Gambar~\ref{fig:confusion_matrix_compare}), yang memvisualisasikan TP, TN, FP, dan FN untuk ketiga model pada 492 sampel data uji. \changed{NGBoost menghasilkan 27 FP (air yang diprediksi layak namun sebenarnya tidak layak) dan 138 FN (air yang diprediksi tidak layak padahal sebenarnya layak), dengan TN=273 dan TP=54}. \changed{Rasio FP:FN NGBoost sebesar $\approx 0{,}20$ menunjukkan bahwa model sangat konservatif dalam memprediksi kelas positif}. Dalam konteks manajemen air minum, \changed{\textit{False Positive} (memprediksi air layak padahal tidak layak)} memiliki implikasi kesehatan yang lebih serius karena dapat menyebabkan penyakit tular-air masif. Sebaliknya, \changed{\textit{False Negative} (memprediksi air tidak layak padahal sebenarnya layak)} mengakibatkan pemborosan biaya pengolahan namun tidak menimbulkan risiko kesehatan langsung. \changed{NGBoost memiliki jumlah FP terendah (27) dibandingkan XGBoost (FP=37, FN=128, TP=64, TN=263) dan \textit{Random Forest} (FP=34, FN=134, TP=58, TN=266). Rendahnya FP NGBoost berarti lebih sedikit sampel air tidak layak yang salah diloloskan, namun hal ini diperoleh dengan mengorbankan recall (lebih banyak FN)}. \changed{Implikasi praktis dari analisis ini adalah bahwa NGBoost menawarkan profil prediksi yang lebih konservatif---meminimalkan risiko meloloskan air tidak layak---sementara XGBoost memberikan keseimbangan FP/FN yang lebih baik untuk kebutuhan deteksi air layak}.
...
Pada dimensi kalibrasi probabilistik, \changed{hasil evaluasi menunjukkan bahwa ketiga model mencapai kalibrasi yang cukup baik pada dataset ini}. \changed{Nilai \textit{Negative Log-Likelihood} (NLL) \textit{Random Forest} sebesar 0,618 merupakan yang terendah, diikuti XGBoost (0,624) dan NGBoost (0,673). Demikian pula, \textit{Expected Calibration Error} (ECE) \textit{Random Forest} (0,042, dihitung dengan $B=10$ \textit{bin}) paling rendah, disusul XGBoost (0,058) dan NGBoost (0,069)}. \changed{Meskipun NGBoost tidak unggul pada metrik kalibrasi agregat, selisih ECE antar ketiga model relatif kecil (maksimum 2,7 poin persentase) dan seluruhnya berada di bawah ambang 0,10 yang umumnya dianggap memadai}.

\changed{Keunggulan utama NGBoost tidak terletak pada nilai ECE yang lebih rendah, melainkan pada kemampuan native-nya dalam memodelkan distribusi penuh probabilitas melalui parameterisasi Bernoulli dan optimasi Natural Gradient berbasis Fisher Information Matrix. Pendekatan ini memungkinkan kuantifikasi ketidakpastian yang prinsipil dan eksplisit, tanpa memerlukan kalibrasi pasca-pelatihan (\textit{post-hoc calibration}) yang diperlukan oleh model deterministik untuk menghasilkan probabilitas yang bermakna}.

Visualisasi kurva kalibrasi pada Gambar~\ref{fig:calibration_curve} secara grafis mengonfirmasi temuan kuantitatif tersebut. \changed{Ketiga model menunjukkan kurva yang relatif mendekati garis kalibrasi sempurna (diagonal abu-abu putus-putus), dengan deviasi yang meningkat pada rentang probabilitas tinggi di mana jumlah sampel per bin lebih sedikit. Kurva \textit{Random Forest} (hijau) dan XGBoost (merah) menunjukkan kesesuaian yang sedikit lebih baik pada rentang probabilitas rendah hingga menengah, sementara NGBoost (biru) menunjukkan kecenderungan sedikit underconfident pada bin-bin tertentu}.
...
\begin{tikzpicture}[scale=4.8]
    ...
    % NGBoost — kurva kalibrasi aktual (biru)
    \draw[blue!80!black, line width=1.8pt]
        (0.10,0.28) -- (0.30,0.30) -- (0.35,0.34) -- (0.45,0.46)
        -- (0.55,0.67) -- (0.65,0.63) -- (0.76,0.88) -- (0.87,1.00) -- (0.95,1.00);
    \filldraw[blue!80!black] (0.10,0.28) circle (0.9pt);
    \filldraw[blue!80!black] (0.30,0.30) circle (0.9pt);
    \filldraw[blue!80!black] (0.35,0.34) circle (0.9pt);
    \filldraw[blue!80!black] (0.45,0.46) circle (0.9pt);
    \filldraw[blue!80!black] (0.55,0.67) circle (0.9pt);
    \filldraw[blue!80!black] (0.65,0.63) circle (0.9pt);
    \filldraw[blue!80!black] (0.76,0.88) circle (0.9pt);
    \filldraw[blue!80!black] (0.87,1.00) circle (0.9pt);
    \filldraw[blue!80!black] (0.95,1.00) circle (0.9pt);
    % XGBoost — kurva kalibrasi aktual (merah)
    \draw[red!75!black, line width=1.8pt]
        (0.16,0.21) -- (0.26,0.25) -- (0.35,0.35) -- (0.45,0.45)
        -- (0.55,0.46) -- (0.65,0.76) -- (0.75,1.00) -- (0.84,1.00);
    \filldraw[red!75!black] (0.16,0.21) circle (0.9pt);
    \filldraw[red!75!black] (0.26,0.25) circle (0.9pt);
    \filldraw[red!75!black] (0.35,0.35) circle (0.9pt);
    \filldraw[red!75!black] (0.45,0.45) circle (0.9pt);
    \filldraw[red!75!black] (0.55,0.46) circle (0.9pt);
    \filldraw[red!75!black] (0.65,0.76) circle (0.9pt);
    \filldraw[red!75!black] (0.75,1.00) circle (0.9pt);
    \filldraw[red!75!black] (0.84,1.00) circle (0.9pt);
    % Random Forest — kurva kalibrasi aktual (hijau)
    \draw[green!55!black, line width=1.8pt]
        (0.17,0.17) -- (0.26,0.26) -- (0.35,0.35) -- (0.44,0.41)
        -- (0.55,0.42) -- (0.65,0.87) -- (0.74,1.00);
    \filldraw[green!55!black] (0.17,0.17) circle (0.9pt);
    \filldraw[green!55!black] (0.26,0.26) circle (0.9pt);
    \filldraw[green!55!black] (0.35,0.35) circle (0.9pt);
    \filldraw[green!55!black] (0.44,0.41) circle (0.9pt);
    \filldraw[green!55!black] (0.55,0.42) circle (0.9pt);
    \filldraw[green!55!black] (0.65,0.87) circle (0.9pt);
    \filldraw[green!55!black] (0.74,1.00) circle (0.9pt);
    % Legend
    \draw[blue!80!black, line width=1.8pt] (0.03,0.96) -- (0.11,0.96);
    \filldraw[blue!80!black] (0.07,0.96) circle (0.8pt);
    \node[right, font=\scriptsize] at (0.12,0.96) {NGBoost (ECE\,=\,$0{,}069$)};
    \draw[red!75!black, line width=1.8pt] (0.03,0.89) -- (0.11,0.89);
    \filldraw[red!75!black] (0.07,0.89) circle (0.8pt);
    \node[right, font=\scriptsize] at (0.12,0.89) {XGBoost (ECE\,=\,$0{,}058$)};
    \draw[green!55!black, line width=1.8pt] (0.03,0.82) -- (0.11,0.82);
    \filldraw[green!55!black] (0.07,0.82) circle (0.8pt);
    \node[right, font=\scriptsize] at (0.12,0.82) {\textit{Random Forest} (ECE\,=\,$0{,}042$)};
    \draw[gray!65, dashed, line width=1.2pt] (0.03,0.75) -- (0.11,0.75);
    \node[right, font=\scriptsize] at (0.12,0.75) {Kalibrasi sempurna};
\end{tikzpicture}%
}
\caption{Kurva Kalibrasi (\textit{Reliability Diagram}) Ketiga Model pada Data Uji (10 \textit{bin}). Sumbu-$x$ menunjukkan rata-rata probabilitas prediksi per \textit{bin}, dan sumbu-$y$ menunjukkan fraksi kelas positif aktual pada \textit{bin} tersebut. Kurva yang lebih mendekati garis diagonal abu-abu putus-putus menandakan kalibrasi yang lebih baik.}
\label{fig:calibration_curve}
\end{figure}

Karakteristik probabilitas prediksi lebih lanjut dianalisis melalui visualisasi distribusi densitas prediksi menggunakan \textit{Kernel Density Estimation} (KDE) pada Gambar~\ref{fig:kde_prob_distribution}. \changed{Distribusi NGBoost menampilkan profil yang lebih tersebar secara kontinu di seluruh rentang $[0,1]$ dibandingkan kedua model baseline yang cenderung terkonsentrasi pada probabilitas rendah. Ketiga model menunjukkan konsentrasi prediksi yang dominan pada rentang $\mu < 0{,}40$ (mencerminkan prevalensi kelas not potable yang mencapai 61\% dari dataset), dengan frekuensi yang lebih rendah pada rentang probabilitas tinggi ($\mu > 0{,}60$). Distribusi NGBoost yang lebih tersebar pada zona menengah ($0{,}30 < \mu < 0{,}60$) dibandingkan model deterministik mengindikasikan bahwa NGBoost lebih transparan dalam mengekspresikan ketidakpastian inheren pada data, sementara model deterministik cenderung mendorong prediksi ke arah keputusan yang lebih tegas}.
...
\subsubsection{Analisis ROC-AUC}
\label{subsubsec:hasil_roc_auc}

Untuk melengkapi evaluasi kalibrasi, dilakukan analisis \textit{Receiver Operating Characteristic} (ROC) yang mengukur kemampuan diskriminatif model secara independen dari ambang keputusan \cite{fawcett2006roc}. Gambar~\ref{fig:roc_curve} menyajikan kurva ROC ketiga model pada data uji.
...
\begin{tikzpicture}[scale=4.8]
    ...
    % Axes
    \draw[->, thick] (-0.03,0) -- (1.09,0)
        node[right, font=\footnotesize] {\textit{False Positive Rate}};
    \draw[->, thick] (0,-0.03) -- (0,1.09)
        node[above, font=\footnotesize] {\textit{True Positive Rate}};
    % Axis ticks
    \draw (0.2,-0.012) -- (0.2,0); \node[below, font=\scriptsize] at (0.2,-0.016) {0,2};
    \draw (0.4,-0.012) -- (0.4,0); \node[below, font=\scriptsize] at (0.4,-0.016) {0,4};
    \draw (0.6,-0.012) -- (0.6,0); \node[below, font=\scriptsize] at (0.6,-0.016) {0,6};
    ...
    \draw[gray!65, dashed, line width=1.2pt] (0,0) -- (1,1);
    % NGBoost ROC curve (AUC = $0{,}651$)
    \draw[blue!80!black, line width=1.8pt]
        (0,0) -- (0.05,0.21) -- (0.10,0.29) -- (0.15,0.35) -- (0.20,0.42)
        -- (0.30,0.51) -- (0.40,0.63) -- (0.50,0.70) -- (0.60,0.77)
        -- (0.70,0.84) -- (0.80,0.92) -- (0.90,0.95) -- (1.0,1.0);
    % XGBoost ROC curve (AUC = $0{,}650$)
    \draw[red!75!black, line width=1.8pt]
        (0,0) -- (0.05,0.23) -- (0.10,0.28) -- (0.15,0.39) -- (0.20,0.46)
        -- (0.30,0.54) -- (0.40,0.60) -- (0.50,0.70) -- (0.60,0.79)
        -- (0.70,0.87) -- (0.80,0.92) -- (0.90,0.96) -- (1.0,1.0);
    % Random Forest ROC curve (AUC = $0{,}667$)
    \draw[green!55!black, line width=1.8pt]
        (0,0) -- (0.05,0.24) -- (0.10,0.28) -- (0.15,0.35) -- (0.20,0.43)
        -- (0.30,0.55) -- (0.40,0.63) -- (0.50,0.69) -- (0.60,0.81)
        -- (0.70,0.85) -- (0.80,0.91) -- (0.90,0.95) -- (1.0,1.0);
    % Legend
    \draw[blue!80!black, line width=1.8pt] (0.55,0.22) -- (0.63,0.22);
    \node[right, font=\scriptsize] at (0.64,0.22) {NGBoost (AUC\,=\,$0{,}651$)};
    \draw[red!75!black, line width=1.8pt] (0.55,0.15) -- (0.63,0.15);
    \node[right, font=\scriptsize] at (0.64,0.15) {XGBoost (AUC\,=\,$0{,}650$)};
    \draw[green!55!black, line width=1.8pt] (0.55,0.08) -- (0.63,0.08);
    \node[right, font=\scriptsize] at (0.64,0.08) {\textit{Random Forest} (AUC\,=\,$0{,}667$)};
    \draw[gray!65, dashed, line width=1.2pt] (0.55,0.01) -- (0.63,0.01);
    \node[right, font=\scriptsize] at (0.64,0.01) {Klasifikasi acak};
\end{tikzpicture}%
}
\caption{Kurva ROC (\textit{Receiver Operating Characteristic}) Ketiga Model pada Data Uji. Sumbu-$x$ menunjukkan \textit{False Positive Rate} dan sumbu-$y$ menunjukkan \textit{True Positive Rate}. Nilai AUC (\textit{Area Under the Curve}) mengukur kemampuan diskriminatif model secara keseluruhan.}
\label{fig:roc_curve}
\end{figure}

Hasil analisis ROC-AUC menunjukkan bahwa \changed{\textit{Random Forest} memiliki nilai AUC tertinggi (0,667), diikuti NGBoost (0,651) dan XGBoost (0,650). Selisih AUC antar ketiga model sangat kecil (maksimum 1,7 poin persentase), mengonfirmasi bahwa kemampuan diskriminatif ketiga model berada pada tingkat yang setara}. \changed{Temuan ini konsisten dengan hasil uji McNemar yang menunjukkan tidak adanya perbedaan signifikan antar model}. \changed{Nilai AUC yang berada pada rentang 0,65--0,67 mengindikasikan kemampuan diskriminatif yang moderat---mencerminkan kompleksitas inheren dataset kualitas air di mana parameter fisikokimia memiliki daya pisah yang terbatas terhadap label kelayakan. Dalam konteks ini, keunggulan NGBoost terletak bukan pada diskriminasi yang lebih baik, melainkan pada kemampuan native-nya dalam memodelkan distribusi penuh probabilitas yang memungkinkan kuantifikasi ketidakpastian secara eksplisit untuk setiap prediksi individual}.
...
\midrule
Sangat yakin (\textit{not potable}) & $\mu < 0{,}20$                & $97 (19{,}7\%)$  & $72{,}2$ \\
Cukup yakin (\textit{not potable})  & $0{,}20 \leq \mu < 0{,}40$   & $222 (45{,}1\%)$  & $67{,}1$ \\
\textbf{Ambigu}                     & $0{,}40 \leq \mu \leq 0{,}60$ & $\textbf{130 (26{,}4\%)}$ & $\textbf{56{,}2}$ \\
Cukup yakin (\textit{potable})      & $0{,}60 < \mu \leq 0{,}80$   & $25 (5{,}1\%)$  & $76{,}0$ \\
Sangat yakin (\textit{potable})     & $\mu > 0{,}80$                & $18 (3{,}7\%)$ & $88{,}9$ \\
\midrule
\textbf{Total}                      & ---                            & \textbf{492 (100\%)} & $\textbf{66{,}5}$ \\
\bottomrule
\end{tabular}
\end{table}

Temuan utama analisis ini menunjukkan bahwa NGBoost mampu membedakan instance berdasarkan tingkat kepercayaan prediksi secara bermakna. \changed{Instance dengan prediksi berkepercayaan tinggi---yaitu $\mu > 0{,}80$ (sangat yakin potable)---mencapai akurasi 88,9\%, sementara instance pada zona ambigu ($0{,}40 \leq \mu \leq 0{,}60$) yang mencakup 26,4\% data uji hanya mencapai akurasi 56,2\%}. \changed{Gradien akurasi yang jelas dari zona ambigu (56,2\%) ke zona sangat yakin potable (88,9\%) menunjukkan bahwa nilai probabilitas NGBoost memiliki korespondensi bermakna terhadap keandalan prediksi aktual}. Fenomena ini sepenuhnya konsisten dengan sifat distribusi Bernoulli (Persamaan~\ref{eq:bernoulli_var}): varians prediksi memang mencapai maksimum pada $\mu = 0{,}5$.

\changed{Distribusi instance yang dominan pada zona cukup yakin not potable (45,1\%) mencerminkan prevalensi kelas mayoritas dalam dataset (61\% not potable). Hanya 8,8\% instance (43 dari 492) berada pada zona kepercayaan tinggi untuk kelas potable ($\mu > 0{,}60$), namun akurasi pada zona tersebut mencapai 76,0--88,9\%, menunjukkan bahwa ketika NGBoost memberikan prediksi positif dengan kepercayaan tinggi, prediksi tersebut sangat andal}.

Temuan ini memiliki implikasi operasional yang langsung dapat diterapkan. Dalam skenario sistem pendukung keputusan berbasis NGBoost, operator pengolahan air dapat menggunakan ambang kepercayaan sebagai panduan: \changed{instance dengan probabilitas berada pada zona sangat yakin ($\mu < 0{,}20$ atau $\mu > 0{,}80$) yang mencakup 23,4\% kasus dan mencapai akurasi rata-rata $74{,}8$\% dapat diproses dengan kepercayaan tinggi, sementara instance pada zona ambigu (26,4\% kasus) dapat ditandai untuk verifikasi laboratorium lanjutan}.

Perbandingan distribusi probabilitas prediksi antara NGBoost dan model \textit{baseline} juga mengungkapkan perbedaan mendasar. \changed{Distribusi prediksi XGBoost dan \textit{Random Forest} cenderung menghasilkan probabilitas yang lebih terkonsentrasi pada rentang sempit, sementara distribusi NGBoost lebih tersebar secara kontinu di seluruh rentang $[0,1]$. Perbedaan ini berakar pada mekanisme algoritmik: NGBoost secara eksplisit memodelkan parameter distribusi melalui Natural Gradient, sehingga menghasilkan gradasi kepercayaan yang lebih halus dan lebih representatif terhadap ketidakpastian inheren pada data parameter fisikokimia air}.
...
Meskipun hasil penelitian menunjukkan keunggulan NGBoost pada dimensi kalibrasi probabilistik, terdapat sejumlah keterbatasan yang perlu diakui secara eksplisit untuk menjaga validitas interpretasi dan memberikan arahan bagi penelitian lanjutan:

\begin{enumerate}
    \item \textbf{Keterbatasan dataset tunggal.} Penelitian ini hanya menggunakan satu dataset publik (\textit{Water Potability Dataset} dari Kaggle) yang berasal dari satu sumber tanpa keragaman geografis atau temporal. Generalisabilitas temuan terhadap kondisi kualitas air di wilayah dengan karakteristik geokimia yang berbeda belum dapat dikonfirmasi.

    \item \textbf{Tidak adanya validasi temporal.} Dataset yang digunakan bersifat statis (\textit{snapshot}) tanpa dimensi waktu. Penelitian ini tidak mengevaluasi stabilitas performa model terhadap pergeseran distribusi data (\textit{distribution shift}) yang mungkin terjadi seiring perubahan musim, kondisi lingkungan, atau degradasi infrastruktur pengolahan air.

    \item \textbf{Tidak adanya pengujian pada kondisi operasional nyata.} Seluruh evaluasi dilakukan dalam kondisi laboratorium komputasional menggunakan dataset yang telah tersedia. Belum dilakukan validasi pada sistem pengolahan air aktual yang melibatkan kendala operasional seperti latensi pengukuran, kegagalan sensor, atau kebutuhan respons waktu-nyata.

    \item \textbf{Ruang pencarian \textit{hyperparameter} yang terbatas.} Optimasi \textit{hyperparameter} dilakukan secara prosedural pada ruang pencarian yang terbatas. Pendekatan optimasi yang lebih komprehensif seperti \textit{Bayesian Optimization} atau \textit{random search} dengan budget evaluasi yang lebih besar berpotensi menghasilkan konfigurasi yang lebih optimal untuk ketiga model.

    \item \textbf{Tidak adanya perbandingan dengan pendekatan probabilistik alternatif.} Penelitian ini membandingkan NGBoost hanya dengan model deterministik (XGBoost dan \textit{Random Forest}). Perbandingan dengan pendekatan probabilistik lain seperti \textit{Bayesian Neural Networks}, \textit{Monte Carlo Dropout}, atau \textit{Deep Ensembles} belum dilakukan, sehingga posisi relatif NGBoost terhadap metode probabilistik lainnya belum dapat ditetapkan secara definitif.
\end{enumerate}
...
\textbf{Pertama}, NGBoost berhasil memodelkan distribusi probabilitas kelayakan air minum dengan kemampuan diskriminatif yang \changed{setara secara statistik} terhadap model deterministik. \changed{NGBoost mencatat akurasi 66,5\% dan \textit{F1-Score} 0,396}---\changed{tidak berbeda signifikan dibandingkan XGBoost (66,5\%, F1 = 0,437) dan \textit{Random Forest} (65,9\%, F1 = 0,409) berdasarkan uji McNemar (seluruh $p > 0{,}75$)}. Hal ini membuktikan bahwa kemampuan NGBoost dalam mengkuantifikasi distribusi penuh probabilitas melalui parameterisasi Bernoulli dan \textit{Natural Gradient} berbasis \textit{Fisher Information Matrix} tidak mengorbankan performa diskriminatif secara berarti.

\textbf{Kedua}, \changed{evaluasi kalibrasi probabilistik menunjukkan bahwa ketiga model mencapai tingkat kalibrasi yang memadai pada dataset ini, dengan ECE berkisar antara 0,042 hingga 0,069}. \changed{Meskipun \textit{Random Forest} mencatat ECE terendah (0,042) dan NLL terendah (0,618) dibandingkan NGBoost (ECE = 0,069; NLL = 0,673), keunggulan utama NGBoost terletak pada kerangka probabilistiknya yang native: NGBoost secara eksplisit memodelkan distribusi penuh melalui optimasi berbasis geometri ruang probabilitas (Riemannian), sehingga menghasilkan output distribusional yang secara prinsipil bermakna untuk kuantifikasi ketidakpastian, tanpa memerlukan asumsi tambahan tentang bentuk distribusi output sebagaimana pada model deterministik}.

\textbf{Ketiga}, analisis ketidakpastian mengidentifikasi bahwa \changed{26,4\%} instance data uji berada pada zona ambigu ($0{,}40 \leq \mu \leq 0{,}60$) dengan akurasi \changed{56,2\%}, sementara \changed{instance pada zona sangat yakin potable ($\mu > 0{,}80$) mencapai akurasi 88,9\%}. \changed{Gradien akurasi yang signifikan antara zona ambigu dan zona berkepercayaan tinggi (selisih 32,7 poin persentase)} menunjukkan bahwa \changed{probabilitas yang dihasilkan NGBoost memiliki korespondensi bermakna terhadap keandalan prediksi aktual}. Kemampuan NGBoost dalam mengekspos perbedaan tingkat kepercayaan ini secara eksplisit merupakan keunggulan fungsional yang tidak dimiliki oleh model deterministik, dan membuka peluang perancangan sistem pendukung keputusan bertingkat yang mengotomasi kasus berkepercayaan tinggi sekaligus menandai kasus ambigu untuk verifikasi lebih lanjut.

Berdasarkan ketiga simpulan tersebut, \changed{NGBoost terbukti menjadi kerangka kerja probabilistik yang kompetitif untuk klasifikasi kelayakan air minum. Meskipun tidak mengungguli model deterministik pada metrik kalibrasi agregat maupun akurasi, NGBoost menyediakan kerangka probabilistik native yang memungkinkan kuantifikasi ketidakpastian secara prinsipil dan eksplisit---sebuah kapabilitas yang tidak tersedia pada model deterministik konvensional. Keunggulan ini menjadi kritis pada skenario pengambilan keputusan berisiko di mana identifikasi instance ambigu sama pentingnya dengan akurasi prediksi itu sendiri}.
